...
public sealed class GigaChatClient : IDisposable
{
    ...
    public const string SystemPrompt =
        ...
        "2. ДЛИНА: Основной текст гадания (до маркера [[ИТОГ]]) — строго от 480 до 700 символов с учётом пробелов. " +
        "Меньше 420 или больше 780 символов — ошибка.\n" +
        "3. ФОРМАТ: Два коротких абзаца или 4–6 предложений: сначала ситуация и смысл, затем совет/поворот. " +
        ...
        "Запрещены бессмысленные цветовые определители и «живописные» краски в метафорах: " +
        "не пиши «малиновые/неоновые/бирюзовые/изумрудные/алые» тени, облака, туманы, стекла и т.п. " +
        "Цвет сам по себе не несёт смысла — опирайся на ситуацию, действие и психологический образ.\n" +
        "5. ЛОГИКА СВЯЗЕЙ: Обязательно оперся минимум на четыре сигнала из контекста (выбирай самые сильные): " +
        "населённый пункт и/или погода, заряд/состояние устройства, мистический образ фото (photo_mystic_tag), слово-якорь, " +
        "тональный модификатор стекла (ball_tint_modifier — это эмоция/состояние, не краска), " +
        "ритм обращений (inquiry_pulse_aura). Остальное — лёгкая модуляция. " +
        "Поле photo_color_palette игнорируй, даже если оно есть. Не называй проценты батареи и частоту вопросов.\n" +
        ...
        "Тревожный / Частый зов: мягче и прямее, опора «здесь и сейчас»; при конфликте с «туманной загадкой» настроения шара приоритет у ясности. " +
        "Не упоминай частоту обращений и не читай нотаций. Если пульс тревожный и батарея 0–40% — ещё прямее, без лишней поэзии.\n" +
        "8. ФИНАЛ: После основного текста отдельной строкой напиши маркер [[ИТОГ]] и сразу за ним ключевую фразу " +
    ...
    public const string UserPreamble =
        "Ниже JSON-контекст гадания. Опирайся минимум на четыре сигнала из dynamic_snapshot " +
        "(якорь, погода и/или место, mystic-образ фото, тональный tint-смысл, пульс или батарея) плюс профиль. " +
        "Не используй названия красок. Не перечисляй поля списком. Ответ — строго по правилам system.\n\n";
    ...
    private async Task<string> ChatAsync(string token, string model, string userJson, CancellationToken cancellationToken)
    {
        var payload = new
        {
            ...
            messages = new[]
            {
                ...
                new { role = "user", content = UserPreamble + userJson },
            },
        };
