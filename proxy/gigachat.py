...
ORACLE_SYSTEM = (
    ...
    "мистический образ фото (photo_mystic_tag), слово-якорь, "
    "тональный модификатор стекла (ball_tint_modifier — эмоция/состояние, не краска), "
    "ритм обращений (inquiry_pulse_aura). Остальное — лёгкая модуляция. "
    "Поле photo_color_palette игнорируй, даже если оно есть. "
...
USER_PREAMBLE = (
    ...
    "(якорь, погода и/или место, mystic-образ фото, тональный tint-смысл, пульс или батарея) "
    "плюс профиль. Не используй названия красок. Не перечисляй поля списком. "
    "Ответ — строго по правилам system.\n\n"
)
